import { DocumentPreview } from "@/components/preview/DocumentPreview";
import { PrintActions } from "@/components/preview/PrintActions";

export default function PreviewPage() {
  return (
    <main className="print-shell min-h-screen bg-ink-950 px-4 py-6 sm:px-6 lg:px-8">
      <div className="no-print mx-auto mb-6 flex max-w-7xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.16em] text-gold-300">
            PDF-preview
          </p>
          <h1 className="mt-2 font-display text-4xl font-semibold text-white">
            Печатная версия документов
          </h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-steel-300">
            PDF формируется стабильным MVP-способом: через HTML-preview и печать
            браузера.
          </p>
        </div>
        <PrintActions />
      </div>
      <DocumentPreview />
    </main>
  );
}
