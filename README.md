...
- AI-панель отображается как заблокированный платный блок и не ломает генератор.
...
Для работы AI-помощника после подключения подписки укажите `OPENAI_API_KEY`.
...
Для работы AI-подсказок после подключения подписки добавьте `OPENAI_API_KEY`.
Без ключа генератор, DOCX/ZIP экспорт и PDF-preview продолжают работать.
...
с нуля и не заменяет юриста. В MVP он закрыт заглушкой подписки: endpoint
возвращает `402`, пока `hasActiveSubscription` не будет подключён к реальной
платежной модели.
...
проверяет подписку, ограничивает частоту запросов и месячный лимит токенов.
...
Helper `src/lib/billing/hasActiveSubscription.ts` пока возвращает `false`.
Когда появятся авторизация и платежный провайдер, в него нужно добавить реальную
проверку активной подписки.
...
- `POST /api/ai/contract-check` — возвращает AI-подсказки после оплаты
  подписки, требует согласие на ПДн и `OPENAI_API_KEY`.
...
- Личный кабинет.
- Платежная система.
- Реальная проверка активной подписки.
