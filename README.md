...
- каталог базовых шаблонов `/templates`;
...
Авторизация, аккаунт, админка, premium-доступ, загрузка своих DOCX-шаблонов и
AI-помощник требуют PostgreSQL/Supabase, потому что пользователи, подписки,
пользовательские шаблоны и usage хранятся в базе.
...
prisma/migrations/20260708120000_extend_document_templates/migration.sql
```
...
3. определяет выбранный `templateId`;
4. проверяет premium-доступ, если выбран premium или пользовательский шаблон;
5. преобразует данные в переменные шаблона;
6. рендерит выбранный DOCX или базовый пакет `contract-template.docx` +
   `act-template.docx` через
   `docxtemplater` + `pizzip`;
7. упаковывает DOCX в ZIP через `JSZip`;
8. возвращает ZIP пользователю.
...
## Каталог и пользовательские шаблоны

Страница `/templates` показывает каталог типов документов:
...
Если в проекте нет проверенных юридических DOCX для конкретного типа, карточка
явно помечается как технический шаблон, который требует проверки специалистом.
Сервис не создаёт юридический текст “из головы”.

На `/generator` первый шаг — выбор шаблона. Для MVP форма остаётся общей, но
выбранный `templateId` передаётся в `/api/generate`, а сервер решает, какой DOCX
рендерить и нужен ли premium-доступ.

Premium-пользователь может открыть `/templates/upload`, загрузить DOCX до 2 МБ
и отправить его на AI-анализ. Endpoint `POST /api/templates/analyze`:

1. проверяет вход пользователя;
2. проверяет active premium;
3. требует согласие на обработку персональных данных;
4. принимает только `.docx`;
5. извлекает plain text через `mammoth`;
6. очищает чувствительные данные;
7. отправляет ограниченный текст в OpenAI;
8. сохраняет metadata, переменные и закрытое содержимое шаблона в
   `DocumentTemplate`.

Для MVP загруженный DOCX хранится в базе в поле `contentBase64`. Для production
лучше вынести файлы в Supabase Storage или S3 с приватными bucket/policy.
Переменные и подсказки формы хранятся в `DocumentTemplate.variables` как JSON.

Страница `/templates/[id]/edit` позволяет владельцу пользовательского шаблона
изменить название, категорию, описание, активность, premium-флаг и JSON-список
переменных. Это не визуальный редактор DOCX: юридический текст файла
редактируется отдельно в Word.
...
- `POST /api/templates/analyze` — анализирует загруженный DOCX и сохраняет
  пользовательский шаблон, требует вход, premium и согласие на ПДн.
- `PATCH /api/templates/[id]` — обновляет настройки пользовательского шаблона,
  доступен только владельцу шаблона.
...
- Хранение пользовательских DOCX в Supabase Storage/S3.
- Сложная CRM.
